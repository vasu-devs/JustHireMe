"""Discovery orchestration — scanning, re-evaluation and their task registry.

Moved out of the API layer so scans are owned by the domain. The API layer
supplies a `manager` whose `.broadcast` fans progress to the UI; nothing here
knows about HTTP.
"""
...
def _default_repo() -> Repository:
    """Process-wide repository, matching api.dependencies.get_repository."""
...
async def run_x_signal_scan(
    ...
    discovery_service = discovery_service or _default_discovery_service()
    kind_filter = "job"
    label = "job leads"
...
async def _run_scan_inner(
    ...
    *,
    ...
    discovery_service = discovery_service or _default_discovery_service()
    ranking_service = ranking_service or _default_ranking_service()
...
async def free_sources_scan(manager, repo: Repository | None = None) -> dict:
    """Run only the keyless sources, on demand."""
    repo = repo or _default_repo()
