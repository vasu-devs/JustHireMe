...
def test_gateway_registry_and_client_request_paths(monkeypatch):
    from gateway.clients.base import BaseServiceClient, ServiceEndpoint, ServiceNotFound, ServiceRegistry, set_service_registry

    endpoint = ServiceEndpoint(name="ranking", base_url="http://service", token="secret", status="healthy")
    registry = ServiceRegistry()
    registry.set(endpoint)
    set_service_registry(registry)

    assert registry.get("ranking") == endpoint
    assert registry.snapshot()["ranking"]["status"] == "healthy"
    ...
    class FakeResponse:
        ...
        def json(self):
            return {"ok": True} if self.status_code < 400 else {"detail": "missing"}
    ...
    class FakeAsyncClient:
        ...
        async def request(self, method, url, headers=None, json=None):
            ...
            assert method == "GET"
            assert url == "http://service/ok"
            return FakeResponse(200)

    monkeypatch.setattr("gateway.clients.base.httpx.AsyncClient", FakeAsyncClient)

    class RankingClient(BaseServiceClient):
        service_name = "ranking"

    assert asyncio.run(RankingClient(endpoint)._request("GET", "/ok")) == {"ok": True}

    class MissingAsyncClient(FakeAsyncClient):
        async def request(self, method, url, headers=None, json=None):
            return FakeResponse(404)

    monkeypatch.setattr("gateway.clients.base.httpx.AsyncClient", MissingAsyncClient)
    with pytest.raises(ServiceNotFound):
        asyncio.run(RankingClient(endpoint)._request("GET", "/ok"))


def test_gateway_supervisor_reports_degraded_services(monkeypatch):
    import gateway.supervisor as supervisor_module
    from gateway.clients import ServiceEndpoint, get_service_registry
    ...
    class FakeAsyncClient:
        ...
        async def get(self, url):
            assert url == "http://ranking/health"
            return FakeResponse()

    monkeypatch.setattr(supervisor_module.httpx, "AsyncClient", FakeAsyncClient)

    disabled = LocalServiceSupervisor(service_names=("ranking",), enabled=False)
    assert asyncio.run(disabled.start()) is disabled.registry
    assert get_service_registry() is None

    degraded = LocalServiceSupervisor(service_names=("ranking",), max_restarts=0)
    degraded.registry.set(ServiceEndpoint(name="ranking", base_url="http://ranking", token="secret", status="healthy"))
    degraded.processes["ranking"] = RunningProcess()

    asyncio.run(degraded.refresh_health())

    endpoint = degraded.registry.get("ranking")
    assert endpoint is not None
    assert endpoint.status == "degraded"
    assert endpoint.last_error == "offline"

    exited = LocalServiceSupervisor(service_names=("ranking",), max_restarts=0)
    exited.registry.set(ServiceEndpoint(name="ranking", base_url="http://ranking", token="secret", status="healthy"))
    exited.processes["ranking"] = ExitedProcess()

    asyncio.run(exited.refresh_health())

    endpoint = exited.registry.get("ranking")
    assert endpoint is not None
    assert endpoint.status == "degraded"
    assert endpoint.restart_count == 1
    assert "process exited" in endpoint.last_error
