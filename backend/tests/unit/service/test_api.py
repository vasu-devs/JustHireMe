...
_TEST_APP_DATA = tempfile.mkdtemp(prefix="jhm-api-tests-")
os.environ["LOCALAPPDATA"] = _TEST_APP_DATA
os.environ["JHM_APP_DATA_DIR"] = _TEST_APP_DATA
...
class TestGraphEndpoint(unittest.TestCase):
    def test_graph_endpoint_uses_profile_snapshot_when_native_graph_is_empty(self):
        from graph_service.stats import graph_stats_payload as graph_stats
...
class TestLeadsEndpoints(unittest.TestCase):
    ...
    def test_manual_customize_rejects_url_only_generation(self):
        ...
        try:
            with mock.patch.object(GenerationOrchestrator, "generate", new=generate_mock):
                resp = post(
...
class TestFormReaderEndpoints(unittest.TestCase):
    ...
    def test_form_read_resolves_candidate_name_from_flat_profile(self):
        ...
        # Use the REAL service over the fake repo so the identity assembly under
        # test actually runs; stub only read_form (the browser call).
        from automation.service import AutomationService

        real_service = AutomationService(fake_repo)
        real_service.read_form = Service().read_form
        app.dependency_overrides[get_repository] = lambda: fake_repo
        app.dependency_overrides[get_automation_service] = lambda: real_service
        try:
    ...
    def test_identity_endpoint(self):
        resp = get("/api/v1/profile/identity")
        self.assertEqual(resp.status_code, 200)
        body = resp.json()
        self.assertIn("email", body)
        # The read must expose every field the settings screen writes, not just
        # the IDENTITY_KEYS subset that PUT /profile/identity persists.
        self.assertIn("full_name", body)
        self.assertIn("current_company", body)
...
class TestGenerateEndpoint(unittest.TestCase):
    def test_generate_waits_for_ready_package(self):
        ...
        with mock.patch.object(GenerationOrchestrator, "generate", new=mock.AsyncMock(return_value=ready_lead)):
    ...
    def test_generate_one_returns_package_when_soft_persistence_steps_fail(self):
        ...
        lead = asyncio.run(
            GenerationOrchestrator(repo, service, job_store).generate("job-1", manager.broadcast)
        )
    ...
    def test_generate_one_rejects_url_only_lead(self):
        ...
        with self.assertRaises(UnprocessableError):
            asyncio.run(
                GenerationOrchestrator(repo, service, JobStore()).generate("url-only", manager.broadcast)
            )
...
class TestDashboardEndpoints(unittest.TestCase):
    """Router-only tests: override the service so these never touch SQLite —
    the query logic itself is covered against a real DB in
    tests/unit/business/test_dashboard.py."""

    def test_overview_returns_the_service_payload(self):
        from api.dependencies import get_dashboard_service

        payload = {
            "headline": {"total": 5, "verified_confirmed": 1, "in_apply_queue": 0,
                         "applied": 0, "interviews": 0, "offers": 0, "rejections": 0},
            "funnel": [{"label": "Scraped", "count": 5, "dropoff_pct": None}],
            "sources": [{"platform": "greenhouse", "total": 5, "verified": 1, "confirmed": 1, "confirm_rate": 100.0}],
            "leads_per_day": [["2026-08-01", 5]],
            "applications_per_day": [],
            "activity": [],
        }
        fake_service = types.SimpleNamespace(overview=mock.AsyncMock(return_value=payload))
        app.dependency_overrides[get_dashboard_service] = lambda: fake_service
        try:
            resp = get("/api/v1/dashboard/overview")
        finally:
    ...
    def test_application_detail_returns_verification_and_answers(self):
        from api.dependencies import get_dashboard_service

        payload = {
            "verify_status": "CONFIRMED", "verify_evidence": "India office confirmed via careers page.",
            "verify_checked_at": "2026-08-01 09:00:00", "answers": "## Why Acme\nBecause it fits.\n",
        }
        fake_service = types.SimpleNamespace(application_detail=mock.AsyncMock(return_value=payload))
        app.dependency_overrides[get_dashboard_service] = lambda: fake_service
        try:
            resp = get("/api/v1/dashboard/applications/abc123")
        finally:
            app.dependency_overrides.pop(get_dashboard_service, None)

        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json(), payload)
        fake_service.application_detail.assert_awaited_once_with("abc123")

    def test_overview_requires_auth(self):
        resp = get("/api/v1/dashboard/overview", auth=False)
        self.assertEqual(resp.status_code, 401)
