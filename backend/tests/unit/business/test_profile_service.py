...
def test_graph_profile_manual_skill_save_updates_snapshot(monkeypatch):
    ...
    assert result["n"] == "Python"
    assert saved["skills"][0]["n"] == "Python"
def test_candidate_resume_parse_only_never_calls_cloud_llm(monkeypatch, tmp_path):
    from profile import ingestor
    ...
    monkeypatch.setattr(
        ingestor,
        "run",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("cloud parser called")),
    )

    parsed = ingestor.parse_only(document_path=str(resume))
    assert parsed.n
    assert any(skill.n.lower() == "python" for skill in parsed.skills)
