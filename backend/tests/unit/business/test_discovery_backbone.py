...
def test_arbeitnow_normalizes_all_current_feed_evidence():
    lead = agg._arbeitnow_lead(
        {
            "slug": "software-intern-bengaluru-1",
            "company_name": "Acme AI",
            "title": "Software Engineering Intern",
            "description": "<p>Build production Python services.</p>",
            "remote": False,
            "url": "https://www.arbeitnow.com/jobs/acme/1",
            "tags": ["Software Development"],
            "job_types": ["Internship"],
            "location": "Bengaluru, India",
            "created_at": 1_787_641_259,
        },
        ["software", "intern"],
    )

    assert lead is not None
    assert lead["location"] == "Bengaluru, India"
    assert lead["workplace"] == "onsite"
    assert lead["active_hint"] == "active"
    assert lead["posted_date"].startswith("2026-")
    assert lead["source_meta"]["slug"] == "software-intern-bengaluru-1"
    assert lead["source_meta"]["job_types"] == ["Internship"]
