...
from discovery.sources.common import current_listing_active_hint
...
def test_only_audited_current_listing_feeds_emit_active_evidence():
    for provider in (
        "arbeitnow", "himalayas", "jobicy", "remoteok", "remotive",
        "themuse", "weworkremotely", "workingnomads",
    ):
        assert current_listing_active_hint(provider) == "active"
    assert current_listing_active_hint("hn_hiring") == "unknown"
    assert current_listing_active_hint("arbitrary-rss") == "unknown"
...
def test_hackernews_monthly_thread_accepts_comments_older_than_global_feed_cutoff():
    ...
    assert hackernews.is_current_hiring_thread_comment(twenty_days_ago)
    assert not hackernews.is_current_hiring_thread_comment(forty_days_ago)
