...
def test_greenhouse_preserves_authoritative_location():
    data = {"jobs": [{
        "title": "Software Engineering Intern",
        "absolute_url": "https://x/1",
        "content": "Global company with offices in India",
...
def test_greenhouse_preserves_requirements_beyond_ranking_excerpt_length():
    decisive_clause = "Applicants must already have unrestricted US work authorization."
    data = {"jobs": [{
        "title": "Software Engineering Intern",
        "absolute_url": "https://x/1",
        "content": ("Build reliable software systems. " * 100) + decisive_clause,
        "location": {"name": "Remote"},
    }]}
    with mock.patch("discovery.sources.ats.json_get", new=_fake_json_get(data)):
        leads = asyncio.run(ats.scrape_greenhouse("acme"))
    assert len(leads[0]["description"]) > 1200
    assert decisive_clause in leads[0]["description"]
...
def test_lever_preserves_authoritative_location():
    data = [{
...
def test_ashby_preserves_authoritative_location():
    data = {"jobs": [{
        "title": "ML Research Intern",
        "jobUrl": "https://x/1",
        "descriptionHtml": "Research role",
        "locationName": "Remote — Worldwide",
    }]}
    with mock.patch("discovery.sources.ats.json_get", new=_fake_json_get(data)):
        leads = asyncio.run(ats.scrape_ashby("acme"))
    assert leads[0]["location"] == "Remote — Worldwide"
