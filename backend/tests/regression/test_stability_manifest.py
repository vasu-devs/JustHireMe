...
def test_degradation_status_is_exposed_to_api_and_frontend():
    health = _read(BACKEND / "api/routers/health.py") + _read(BACKEND / "system/health.py")
    ...
    # The UI reaches the backend through the service layer (src/api), so the
    # contract App.tsx must keep is the client call, not a literal URL.
    assert "healthApi.subsystems" in app
    assert '"/api/v1/health/subsystems"' in _read(ROOT / "src/api/health.ts")
