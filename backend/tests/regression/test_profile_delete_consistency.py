...
def test_graph_stats_endpoint_is_wired_to_apply_deletions():
    """graph_stats_payload must run the snapshot + embedding through the filters."""
    src = (BACKEND / "graph_service" / "stats.py").read_text(encoding="utf-8")
    tree = ast.parse(src)
    fn = next(
        node for node in ast.walk(tree)
        if isinstance(node, ast.AsyncFunctionDef) and node.name == "graph_stats_payload"
    )
    called = {
        node.func.id
