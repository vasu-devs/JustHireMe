...
def test_actuate_job_enriches_lead_via_get_lead_for_fire():
    ...
    manager = FakeManager()
    service = automation_service.AutomationService(repo)
    # The flow under test lives on the service; stub only the two collaborators
    # so the enrichment path itself is exercised for real.
    fake = FakeService()
    service.get_lead_for_fire = fake.get_lead_for_fire
    service.submit_application = fake.submit_application

    with mock.patch.object(automation_service, "fire_blocker", return_value=(0, "")):
        asyncio.run(service.actuate("job-9", manager.broadcast, job_store=job_store))
