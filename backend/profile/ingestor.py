...
def run(raw: str = "", pdf: str | None = None) -> C:
    ...
    try:
        result = call_llm(
            ...
            '  \"exp\": [{\"role\": \"Job Title\", \"co\": \"Company Name\", \"period\": \"Jan 2022 - Present\", \"location\": \"City, Region if stated for this role (else empty)\", \"d\": \"responsibilities and achievements\", \"s\": [\"skill1\", \"skill2\"]}],\n'
            '    \u2014 one entry per job; include every role, not just recent ones\n'
            '    \u2014 \"period\": copy the dates exactly as written (both endpoints, e.g. \"May 2020 - Aug 2022\"); never shorten them\n'
