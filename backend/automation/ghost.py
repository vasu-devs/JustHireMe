"""Ghost mode — the unattended scan → score → generate → apply cycle.

Business orchestration that a scheduler triggers on a timer. It lives here, not
in ``api/scheduler.py``, because nothing about it is transport: the API layer
only owns the APScheduler instance and hands this a `manager` to broadcast with.
"""
...
from discovery.orchestrator import run_free_source_scan, run_x_signal_scan
...
from automation.service import fire_blocker
...
def create_ghost_tick(manager):
    async def ghost_tick():
        repo = _default_repo()
        automation_service = _default_automation_service(repo)
        discovery_service = _default_discovery_service()
        ranking_service = _default_ranking_service()
        generation_service = _default_generation_service()
        ...
        try:
            ...
            logging.getLogger(__name__).warning('suppressed exception in automation/ghost.py:ghost_tick: %s', exc)
