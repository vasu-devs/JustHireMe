...
- Much broader first-party discovery. JustHireMe now understands 43 source families across 314 production targets, from startup ATS boards through large-company career systems, including Greenhouse, Lever, Ashby, Workday, SmartRecruiters, Recruitee, Personio, SuccessFactors, Jibe/iCIMS Attract, official company pages, aggregators, community feeds, and other keyless sources. Live market validation exercised 60,824 raw observations and resolved them into 25,697 canonical opportunities.
- Applicability and liveness are now first-class data. Every opportunity can carry source provenance, canonical identity and aliases, observed/posted/expiry timestamps, remote and location evidence, India eligibility, employment and internship type, experience and graduation constraints, compensation, skills, qualifications, responsibilities, application route, verification status, and rejection reasons. Stale, duplicate, inaccessible, unpaid, bond-heavy, senior, or location-ineligible roles are kept out of the primary path.
- Safer canonical opportunity storage and refresh. New migrations and services preserve source observations, aliases, candidate-specific decisions, outcomes, paid-provider governance, and refresh history while keeping the app local-first. Repeated scans update the same opportunity instead of multiplying copies, and ambiguous records remain reviewable rather than being presented as verified truth.
- Interview-focused ranking and workflow. Candidate profiles, shortlist bands, transparent decision reasons, application status, outcome tracking, dashboards, and opportunity APIs connect discovery to the existing CRM, document generation, and follow-up workflow. The default experience remains human-reviewed; browser automation and auto-apply stay experimental and opt-in.
- Release validation was expanded with a reorganized unit/regression/vector test suite, opportunity contract and applicability harnesses, strict full-market audits, installed-database integrity checks, and web UI smoke coverage. The final local gate passed 1,874 backend tests (14 skipped), 89 frontend tests, the production frontend build, and 5 Rust tests.
