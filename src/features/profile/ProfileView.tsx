...
import { entryTitle, mergeProfileWithGraphFallback, normalizeProfileResponse } from "./profileUtils";
...
export function ProfileView({ api, setView, stats }: { api: ApiFetch; setView: (view: View) => void; stats?: GraphStats }) {
  const [profile, setProfile] = useState<ProfileData>(() => normalizeProfileResponse({}));
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setProfile(normalizeProfileResponse(mergeProfileWithGraphFallback(body, stats)));
      setError(null);
    } catch (cause) {
      ...
      <main className="profile-evidence-column">
        <section className="profile-dossier-section profile-skills-section">
          ...
          <div className="profile-skill-cloud">
            {skills.length ? skills.map((item, index) => <span key={`${entryTitle(item)}-${index}`}><i>{String(index + 1).padStart(2, "0")}</i>{entryTitle(item)}</span>) : <button onClick={() => setView("ingestion")}><DemoIcon name="plus" />Add your first verified skill</button>}
          </div>
        </section>
        ...
        <section className="profile-dossier-section">
          ...
          <div className="profile-timeline">
            {experience.length ? experience.map((item, index) => <article key={`${entryTitle(item)}-${index}`}><i /><div><span>{entryDetail(item, ["period"]) || `Chapter ${String(index + 1).padStart(2, "0")}`}</span><h4>{entryTitle(item)}</h4><strong>{entryDetail(item, ["co", "company"])}</strong><p>{entryDetail(item, ["d", "description"]) || "Add the scope and outcome so generated applications stay grounded."}</p></div></article>) : <ProfileEmpty title="Your timeline is empty" body="Add a role, freelance chapter, or meaningful apprenticeship." onClick={() => setView("ingestion")} />}
          </div>
        </section>
      </main>
