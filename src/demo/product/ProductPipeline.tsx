...
export function ProductPipeline({ jobs, onSelect, onMove }: { jobs: DemoJob[]; onSelect: (job: DemoJob) => void; onMove: (id: number, stage: DemoJob["stage"]) => void }) {
  ...
  return <div className="pipeline-view product-enter">
    <div className="view-toolbar"><div><span className="product-eyebrow">Your application board</span><h2>Roles in <em>motion</em></h2><span className="toolbar-scribble">keep these close ✦</span></div><div className="view-toolbar-actions"><label><DemoIcon name="search" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Filter roles…" /></label><button><DemoIcon name="tune" />Filter</button><button className="toolbar-primary"><DemoIcon name="plus" />Add role</button></div></div>
    <div className="pipeline-summary"><span><i />8 active roles</span><span><mark className="diary-highlight pink">3 ready for review</mark></span><span>2 follow-ups due</span><div className="board-presence"><b>VS</b><b>AI</b><small>Review session</small></div><button>Board <kbd>B</kbd></button><button>List <kbd>L</kbd></button></div>
